import { generateDatesfromBeginning } from "../utils/generate-dates-from-year-beginning"
        ...
        <div className="w-full flex">
            <div className="grid grid-rows-7 grid-flow-row gap-3">
                {weekDays.map((weekDay, i) => {
                    return (
                        <div className="text-zinc-400 text-xl h-10 w-10 font-bold flex items-center justify-center" key={`${weekDay}-${i}`}>
                            {weekDay}
                        </div>
                    )
                })}
